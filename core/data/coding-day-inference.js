 ...
 * @param {Array} commits - All commits
 * @returns {Object} - { coding_day_start_hour, confidence, histogram }
 */
export function inferCodingDayStart(commits) {
  if (commits.length === 0) {
    return { coding_day_start_hour: 4, confidence: 0, histogram: new Array(24).fill(0) };
  }
  const histogram = generateUTCHourHistogram(commits);
  const valley = findActivityValley(histogram);
  const coding_day_start_hour = Math.floor((valley.start_hour + valley.length / 2)) % 24;
  return {
    coding_day_start_hour,
