...
export function determineSessionThreshold(commits, userConfig) {
  ...
  const intervalMinutes = intervals
    .map((i) => i.interval_minutes)
    .sort((a, b) => a - b);
  const jenksThreshold = findJenksBreak(intervalMinutes);
  const elbowThreshold = findElbowPoint(intervalMinutes);
  const gapThreshold = findLargestGap(intervalMinutes);
  const outlierThreshold = findOutlierThreshold(intervalMinutes);
  const bimodalThreshold = findBimodalSplit(intervalMinutes);
  const analysis = {
    jenks: jenksThreshold,
    elbow: elbowThreshold,
    gap: gapThreshold,
    outlier: outlierThreshold,
    bimodal: bimodalThreshold,
    dataStats: {
      count: intervalMinutes.length,
      min: Math.min(...intervalMinutes),
      max: Math.max(...intervalMinutes),
      median: intervalMinutes[Math.floor(intervalMinutes.length * 0.5)],
      p75: intervalMinutes[Math.floor(intervalMinutes.length * 0.75)],
      p90: intervalMinutes[Math.floor(intervalMinutes.length * 0.9)],
      p95: intervalMinutes[Math.floor(intervalMinutes.length * 0.95)],
    },
  };
  const recommendedThreshold = selectOptimalThreshold(
    analysis,
    intervalMinutes
  );
  return {
...
function findJenksBreak(values) {
  if (values.length < 10) return null;
  const relevantValues = values.filter((v) => v >= 5 && v <= 180);
  if (relevantValues.length < 5) return null;
  const candidates = [];
  const step = (Math.max(...relevantValues) - Math.min(...relevantValues)) / 20;
  for (let breakPoint = 15; breakPoint <= 120; breakPoint += step) {
    ...
    const totalVariance = calculateVariance(relevantValues);
    const gvf =
...
function findElbowPoint(values) {
  if (values.length < 10) return null;
  const logValues = values.filter((v) => v > 0).map((v) => Math.log10(v));
...
function findLargestGap(values) {
  ...
  const relevant = values.filter((v) => v >= 5 && v <= 180);
...
function findOutlierThreshold(values) {
  if (values.length < 10) return null;
  const median = values[Math.floor(values.length * 0.5)];
  const mad = calculateMAD(values, median);
  if (mad === 0) return null;
  const threshold = median + 2.5 * mad * 1.4826; // 1.4826 is scaling factor for MAD
  return Math.max(15, Math.min(120, threshold));
}

function findBimodalSplit(values) {
  if (values.length < 20) return null;
  const logValues = values.filter((v) => v > 0).map((v) => Math.log10(v));
  const minLog = Math.min(...logValues);
  const maxLog = Math.max(...logValues);
  const binCount = Math.min(20, Math.floor(values.length / 5));
...
function selectOptimalThreshold(analysis, values) {
  const candidates = [
    { method: "jenks", value: analysis.jenks, weight: 0.25 },
    { method: "elbow", value: analysis.elbow, weight: 0.25 },
    { method: "gap", value: analysis.gap, weight: 0.2 },
    { method: "outlier", value: analysis.outlier, weight: 0.15 },
    { method: "bimodal", value: analysis.bimodal, weight: 0.15 },
  ].filter((c) => c.value !== null && c.value >= 15 && c.value <= 120);
  if (candidates.length === 0) {
    const p75 = analysis.dataStats.p75;
    const p90 = analysis.dataStats.p90;
    return {
      value: Math.max(30, Math.min(90, Math.sqrt(p75 * p90))),
...
function smoothHistogram(histogram, windowSize = 3) {
