...
async function processUserPeriods(username, periods, clusterThreshold) {
  ...
  const periodMetrics = {}; // Original variable name kept
  const periodSummaries = {}; // New variable for summaries only
  const allCommits = [];
  for (const period of periods) {
    ...
    periodMetrics[period.name] = metrics; // Keep original behavior
