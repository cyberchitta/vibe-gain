...
async function main() {
  ...
  for (const user of params.GITHUB_USERNAMES) {
    ...
    if (user.day_boundary_utc !== undefined && !forceUpdate) {
      console.log(
        `  Already has day_boundary_utc: ${user.day_boundary_utc} (use --force to update)`
      );
      continue;
    }
    ...
    if (commits.length === 0) {
      console.log(
        `  No commits found, skipping (cannot infer day_boundary_utc without data)`
      );
      continue;
    }
    ...
    console.log(
      `  Inferred day_boundary_utc: ${inference.day_boundary} UTC`
    );
    console.log(`  Confidence: ${(inference.confidence * 100).toFixed(1)}%`);
    console.log(
      `  Activity valley: ${
        inference.histogram[inference.day_boundary]
      } commits at this hour`
    );
    user.day_boundary_utc = inference.day_boundary;
    updated = true;
  }
  if (updated) {
    ...
    console.log(
      "\nNo updates made. Users either already have day_boundary_utc set or have no commit data."
    );
  }
}
