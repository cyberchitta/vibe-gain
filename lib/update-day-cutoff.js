...
async function main() {
  ...
  for (const user of params.GITHUB_USERNAMES) {
    ...
    console.log(
      `  Inferred day_cutoff_utc: ${inference.coding_day_start_hour} UTC`
    );
    console.log(`  Confidence: ${(inference.confidence * 100).toFixed(1)}%`);
    console.log(
      `  Activity valley: ${
        inference.histogram[inference.coding_day_start_hour]
      } commits at this hour`
    );
    user.day_cutoff_utc = inference.coding_day_start_hour;
    updated = true;
  }
  if (updated) {
