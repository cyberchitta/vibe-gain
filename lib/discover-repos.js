...
async function withRetry(fn, maxRetries = 3, baseDelay = 1000) {
    ...
    try {
      ...
      if (
        ...
        if (error.response?.headers?.["x-ratelimit-reset"]) {
          // Use reset time from header if available (REST API)
          const resetTime =
  ...
  console.error(
    `Max retries (${maxRetries}) reached, failing with error:`,
    lastError.message
  );
  throw lastError;
}
...
function splitDateRange(start, end) {
  ...
  while (currentStart <= endDate) {
    ...
    currentStart = new Date(subPeriodEnd.getTime() + 1000); // Move to next day
  }
  return subPeriods;
}
...
async function discoverRepositoriesForPeriod(username, period) {
  ...
  try {
    ...
    if (periodDays > 30) {
      console.log(
        `Period exceeds 30 days (${periodDays} days), splitting into monthly sub-periods...`
      );
        ...
        while (hasMoreResults) {
          try {
            ...
            if (
              ...
              console.warn(
                `GitHub 1,000-result cap hit for ${searchQuery}. Relying on sub-periods to capture more repos.`
              );
            } else {
