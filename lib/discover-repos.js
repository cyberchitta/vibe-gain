...
function splitDateRange(start, end) {
  ...
  while (currentStart <= endDate) {
    const subPeriodEnd = new Date(currentStart);
    subPeriodEnd.setDate(subPeriodEnd.getDate() + 7);
    if (subPeriodEnd > endDate) {
      subPeriodEnd.setTime(endDate.getTime());
    }
    subPeriods.push({
      start: currentStart.toISOString().split("T")[0],
      end: subPeriodEnd.toISOString().split("T")[0],
    });
    currentStart = new Date(subPeriodEnd.getTime() + 1000);
  }
  return subPeriods;
}
...
async function discoverRepositoriesForPeriod(username, period) {
  ...
  try {
    ...
    if (error.code === "ENOENT") {
      console.log(`No existing repository list found at ${reposFilePath}, proceeding with discovery...`);
    } else {
  ...
  try {
    ...
    let periodsToSearch = [{ start: period.start, end: period.end }];
    let allRepos = new Set();
    let failedSubPeriods = [];
    const periodDays =
      (new Date(period.end) - new Date(period.start)) / (1000 * 60 * 60 * 24);
    if (periodDays > 7) {
      console.log(
        `Period exceeds 7 days (${periodDays} days), splitting into weekly sub-periods...`
      );
      periodsToSearch = splitDateRange(
        `${period.start}T00:00:00Z`,
        `${period.end}T23:59:59Z`
      );
    }
    for (const subPeriod of periodsToSearch) {
      console.log(
        `Processing sub-period: ${subPeriod.start} to ${subPeriod.end}`
      );
      await checkRateLimit();
      for (const searchQueryTemplate of searchQueries) {
        const searchQuery = searchQueryTemplate.replace(
          `committer-date:${period.start}..${period.end}`,
          `committer-date:${subPeriod.start}..${subPeriod.end}`
        );
        let page = 1;
        ...
        try {
          while (hasMoreResults) {
            ...
            if (searchResult.data.incomplete_results) {
              console.warn(
                `Warning: GitHub search results incomplete for ${searchQuery} (page ${page}, capped at 1,000). Using sub-periods to mitigate.`
              );
            }
            if (searchResult.data.items && searchResult.data.items.length > 0) {
              ...
              for (const item of searchResult.data.items) {
                if (item.repository) {
                  allRepos.add(item.repository.full_name);
                }
              }
            }
            hasMoreResults =
          ...
          if (
            ...
            searchError.message.includes(
              "Only the first 1000 search results are available"
            )
          ) {
            console.warn(
              `GitHub 1,000-result cap hit for ${searchQuery}. Using sub-periods to mitigate.`
            );
            hasMoreResults = false;
          } else if (
            searchError.status === 403 &&
            searchError.message.includes("rate limit exceeded")
          ) {
            console.log(
              `Rate limit exhausted for ${searchQuery}, queuing sub-period for retry...`
            );
            failedSubPeriods.push(subPeriod);
            hasMoreResults = false;
          } else {
            console.error(
              `Unexpected error in search query ${searchQuery} (page ${page}):`,
              searchError.message
            );
            hasMoreResults = false;
          }
      ...
      if (periodsToSearch.length > 1) {
        console.log(
          `Pausing 30s before next sub-period to respect rate limits...`
        );
        await new Promise((resolve) => setTimeout(resolve, 30000));
      }
    }
    if (failedSubPeriods.length > 0) {
      console.log(`Retrying ${failedSubPeriods.length} failed sub-periods...`);
      for (const subPeriod of failedSubPeriods) {
        console.log(
          `Retrying sub-period: ${subPeriod.start} to ${subPeriod.end}`
        );
        await checkRateLimit();
        for (const searchQueryTemplate of searchQueries) {
          const searchQuery = searchQueryTemplate.replace(
          ...
          let page = 1;
          let hasMoreResults = true;
          console.log(`Executing retry search query: ${searchQuery}`);
          try {
            while (hasMoreResults) {
              const searchResult = await withRetry(() =>
                octokit.search.commits({
              ...
              hasMoreResults =
                searchResult.data.items &&
                searchResult.data.items.length === 100;
              page++;
            }
          } catch (searchError) {
            if (
              ...
              console.error(
                `Failed retry for ${searchQuery} (page ${page}):`,
                searchError.message
              );
              hasMoreResults = false;
            }
    ...
    console.log(
      `Found ${allRepos.size} unique repositories from commit search across all sub-periods`
    );
    allRepos.forEach((repo) => discoveredRepos.add(repo));
  } catch (error) {
