  ...
  try {
    ...
    for (const [timestamp, commitGroup] of Object.entries(commitsByTimestamp)) {
      if (commitGroup.length > 1) {
        ...
        const hasRepoConflict = repos.some((repo) => precedenceMap.has(repo));
        if (hasRepoConflict) {
          duplicateGroups++;
          ...
          dedupedCommits.push(selectedCommit);
          removedCommits += commitGroup.length - 1;
          console.log(
            `    Timestamp ${timestamp}: kept ${selectedCommit.repo}, removed ${
              commitGroup.length - 1
            } commits`
          );
        } else {
