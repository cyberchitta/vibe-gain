...
import { createNaturalBins, hasNaturalBuckets } from "../../core/data/bucketing.js";

export function formatBoxPlotTable(periodsData, metricLabel, metricId) {
  const rows = [
...
export function formatHistogramTable(periodsData, metricLabel, metricId) {
    ...
    if (metricId === "commits_by_hour_of_day") {
      return {
        ...
        bins: metricData.map((count, hour) => ({
          ...
          percentage:
            metricData.length > 0
              ? (count / metricData.reduce((a, b) => a + b, 0)) * 100
              : 0,
        })),
      };
    }
  ...
  Array.from(allBinLabels).forEach((label) => {
    const values = binsPerPeriod
      .map(({ bins }) => {
