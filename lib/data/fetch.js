...
export async function fetchCommits(
  ...
  try {
    ...
    if (commits.length > 0) {
      ...
      const requiredStartDate = new Date(expandedStartDate);
      const requiredEndDate = new Date(expandedEndDate);
      if (
        cacheStartDate > requiredStartDate ||
        cacheEndDate < requiredEndDate
      ) {
        console.log(
    ...
    const repoMetadataPath = path.join(
      path.dirname(jsonPath),
      `repo-metadata-${periodName}.json`
    );
    let repoMetadata = {};
    try {
      const repoMetadataJson = await fs.readFile(repoMetadataPath, "utf8");
      repoMetadata = JSON.parse(repoMetadataJson);
      console.log(`Loaded repository metadata from ${repoMetadataPath}`);
    } catch (metadataError) {
      console.log(`No repository metadata found at ${repoMetadataPath}`);
    }
    const expectedTotalCount = await getTotalCommitCount(
      username,
      startDate,
    ...
    const cacheStats = {
      ...
      username,
      period: { startDate, endDate },
      expandedPeriod: { expandedStartDate, expandedEndDate },
      source: "cache",
      cachedCommitCount,
      expectedTotalCount,
      privateCommits: commits.filter((row) =>
        isPrivateCommit(row, repoMetadata)
      ).length,
      forkCommits: commits.filter((row) => isForkCommit(row, repoMetadata))
        .length,
    ...
    if (commitData.length > 0) {
      ...
      console.log(`Saved repository metadata to ${repoMetadataPath}`);
    } else {
      console.log(
